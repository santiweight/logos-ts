...
const ClaudeLogo = () => (
  <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" style={{ verticalAlign: "-2px", flexShrink: 0 }}>
  ...
  return (
    <aside className="comment-sidebar">
      ...
      <div className="cs-header">
        <span className="cs-title"><ClaudeLogo /> Claude</span>
      </div>

      {goal == null ? (
        <div className="cs-idle">
          <div className="cs-idle-hint">Select a workspace from the rail, or start a new thread below.</div>
          <div className="cs-composer">
            <textarea
              value={newComment}
              onChange={(e) => setNewComment(e.target.value)}
              onKeyDown={(e) => {
                if ((e.metaKey || e.ctrlKey) && e.key === "Enter") submitNewComment()
              }}
              placeholder="Ask Claude anything..."
            />
            <button type="button" disabled={newComment.trim().length === 0} onClick={submitNewComment}>
              Send
            </button>
          </div>
        </div>
