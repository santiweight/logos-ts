...
export const FRONTEND_BACKEND_SPLIT_GUIDANCE =
  `Keep UI code focused on presentation: rendering, layout, styling, event wiring, local interaction state, and small display-only formatting. Put substantive data behavior in backend/domain/data modules or server-side helpers instead of React components: filtering, sorting, ranking, scoring, searching, parsing, validation, aggregation, authorization-sensitive decisions, persistence, and transformations that may grow with data size. If a page or component needs that behavior, expose a named reusable helper from the existing backend/domain module and have the UI call it or pass through its result; do not hide business logic or potentially heavy data work inside JSX/component-local helpers.`
...
export function buildImplPrompt(context: string, sandbox: string, goalLine: string, verifyNote: string): string {
  return `${context}\n\n${sandbox}` +
    ...
    `Keep exported signatures stable unless a change requires otherwise; reuse existing helpers; make it typecheck. ${FRONTEND_BACKEND_SPLIT_GUIDANCE} ${SEARCH_RANKING_GUIDANCE} ${verifyNote}\n\n` +
...
export function buildArchImplementationPrompt(context: string, sandbox: string, goalLine: string, verifyNote: string): string {
  return `${context}\n\n${sandbox}` +
    ...
    `Address these change requests:\n${goalLine}\n\n` +
    `Reuse existing helpers and data flow, make it typecheck, and keep the app behavior intuitive for the user-facing workflow. ${FRONTEND_BACKEND_SPLIT_GUIDANCE} ${SEARCH_RANKING_GUIDANCE} ${verifyNote}\n\n` +
