...
const STORY_SNAPSHOT_REQUIRED_PACKAGES = ["playwright", "vite"] as const
...
export function missingStorySnapshotDependencies(frontendDir: string): string[] {
  ...
  return STORY_SNAPSHOT_REQUIRED_PACKAGES.filter((pkg) => {
    try {
      if (!existsSync(join(resolve(frontendDir), "node_modules", pkg))) return true
...
export function ensureStorySnapshotTest(root: string, stories: StoryEntry[], dirs: StorybookDirs): StorySnapshotTestResult {
  ...
  const testSource = [
    ...
    "type ViteServer = { listen(): Promise<void>; close(): Promise<void>; resolvedUrls?: { local: string[] } }",
    ...
    "  beforeAll(async () => {",
    "    const viteModule = await import(pathToFileURL(projectRequire.resolve(\"vite\")).href) as { createServer: (config: unknown) => Promise<ViteServer> }",
    "    server = await viteModule.createServer({",
    "      root: projectRoot,",
    "      configFile: false,",
