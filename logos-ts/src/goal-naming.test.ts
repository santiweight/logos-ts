...
describe("goal naming", () => {
  it("builds a constrained Haiku prompt from the comment and target context", () => {
    ...
    expect(prompt).toContain("not the method")
    expect(prompt).toContain("Do not echo generic verbs")
    ...
    expect(prompt).toContain("Title: Collapse Advanced Filters")
    expect(prompt).toContain("Title: UserCard Loading Error States")
    expect(prompt).toContain("Title: Fix Missing Post 404")
    expect(prompt).toContain("Now name the following chat:")
    expect(prompt).toContain("User comment: The filter sidebar is too noisy")
  })
